# -*- coding: utf-8 -*-
# Güncelleme Tarihi: 08.07.2025 09:00
# Yapılan Değişiklikler:
# 1. Kullanıcının isteği doğrultusunda, arayüzü basitleştirmek için 'Kategori Eşleştirme' ve 'Marka Eşleştirme' bölümleri tamamen kaldırıldı.
# 2. 'AttributeError' hatasını çözmek için, Trendyol'dan kategori ve marka listelerini çeken kodlar kaldırıldı.
# 3. Kod, sadece temel ERP ve API ayarları ile Şube yönetimine odaklanacak şekilde yeniden düzenlendi.
...
from ...repositories import branch_repo, settings_repo
...
class SettingsScreen(Screen):
    def __init__(self, **kwargs):
        ...
        erp_tab = TabbedPanelHeader(text='ERP & Şube Ayarları')
    ...
    def build_erp_settings_tab(self):
        ...
        erp_layout.add_widget(create_section_header("2. Şube Ayarları"))
    ...
    def _run_erp_test_and_load(self, erp_config_override):
        synchronizer.update_gui_status("ERP bağlantısı test ediliyor...")
        erp_handler = ERP12Handler(erp_config_override=erp_config_override)
        is_success, message = erp_handler.test_connection()
        if not is_success:
            synchronizer.update_gui_status(f"HATA: ERP bağlantısı başarısız! {message}")
            return
        
        synchronizer.update_gui_status("ERP Bağlantısı başarılı. ERP listeleri çekiliyor...")
        self.erp_price_lists_data = erp_handler.get_all_erp_price_lists()
        locations = erp_handler.get_all_erp_locations()
        
        Clock.schedule_once(lambda dt: self.populate_dynamic_settings(self.erp_price_lists_data, locations))

    def populate_dynamic_settings(self, price_lists, locations):
        if price_lists:
    ...
    def populate_branch_settings_ui(self, locations):
        ...
        header.add_widget(Label(text="ERP Şubesi", bold=True))
        header.add_widget(Label(text="Aktif?", bold=True, size_hint_x=0.2))
        header.add_widget(Label(text="Buffer", bold=True, size_hint_x=0.2))
        header.add_widget(Label(text="Trendyol Mağaza ID", bold=True, size_hint_x=0.4))
        ...
        for loc in locations:
            ...
            buf = create_styled_textinput(text=str(settings.get('stock_buffer', 0)), size_hint_x=0.2, input_filter='int', halign='center')
            store_id_input = create_styled_textinput(text=str(settings.get('trendyol_store_id', '')), size_hint_x=0.4)
            row.add_widget(Label(text=loc['AD']))
            row.add_widget(chk)
            row.add_widget(buf)
            row.add_widget(store_id_input)
            self.branch_settings_layout.add_widget(row)
            self.branch_setting_widgets[loc_id] = {'checkbox': chk, 'buffer': buf, 'store_id': store_id_input, 'name': loc['AD']}
    ...
    def _run_api_test(self, api_config):
        synchronizer.update_gui_status("Trendyol API bağlantısı test ediliyor...")
        try:
            api_config['base_url'] = "https://stageapi.tgoapis.com" if api_config['test_mode_enabled'] else "https://api.tgoapis.com"
            ...
            synchronizer.update_gui_status(log_message)
        ...
        try:
            ...
            for loc_id, widgets in self.branch_setting_widgets.items():
                branch_data = {
                    ...
                    'trendyol_store_id': widgets['store_id'].text.strip(),
                    'erp_price_list_id': self.selected_price_list_id
                }
                branch_repo.add_or_update_branch_mapping(branch_data)

            synchronizer.update_gui_status("Tüm ayarlar başarıyla kaydedildi.")
        except Exception as e:
            logger.error("Ayarlar kaydedilirken hata oluştu.", exc_info=True)
            synchronizer.update_gui_status(f"HATA: Ayarlar kaydedilemedi - {e}")
